...
TestAjaxProtocol = (function() {
	 ...
	 * @create   : 2012-7-18
	 */
    var testAjaxValidateOptions   = function(options) {
    ...
    var findParamKey = function(urlParam){
        var paramList = URLParam.getParamList();
        var pmKey = "";
        //根据url匹配
        $.each(paramList, function(key, value){
            if(urlParam.url == value.url){
                pmKey = key;
                return ;
            }
        });
        //console.error("not found the url:" + urlParam.url);
        return pmKey;
    }
	 ...
	 * @create   : 2012-7-18
	 */
	var getTestAjaxOptions = function(urlParam){
		var opts = {};
	 ...
	 * @create   : 2012-7-19
	 */
    var testAjaxRequest = function(options) {

        //组装成新的json url。
        var key = findParamKey(options);
        options.url = options.fileDir + "/" + key.replace(".","/") + ".json";
        ...
        var result = $.ajax({
			...
			dataType     : "json", //'jsonp',	//默认是系统智能猜测(xml, json, script, or html)。
	...
	var testAjaxRegisterEvent = function(options){
		testAjaxRequest(options);
		setTimeout(function(){
			ajaxRegisterEvent(options);
		}, options.refreshTime || 2000);
	};
	
	Protocol.registerRequest("testAjax", {
		type : "testAjax",
		getOptions : getTestAjaxOptions,
		validateOptions : testAjaxValidateOptions,
		request : testAjaxRequest,
		registerEvent : testAjaxRegisterEvent
	});
